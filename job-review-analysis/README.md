...
본 프로젝트는 데이터 분석 부트캠프에서 6명이 수행한 팀 프로젝트다. 이 저장소는 제가 담당한 **기업 리뷰 전처리, 리뷰 점수화·등급화, 최종 분석 테이블 생성과 사이트 구현**을 중심으로 재구성했다.
...
프로젝트 참여자는 총 6명이며, 포트폴리오에서는 본인을 제외한 구성원의 이름을 역할명으로 익명화했다.

| 구성원 | 담당 영역 |
|---|---|
| 팀장 | 서비스 설계, 사이트 구현 |
| 팀원 A | 외부 데이터 수집, 채용공고 분석 |
| 팀원 B | 채용공고 데이터 전처리, 채용공고 분석 |
| **배지환(본인)** | **리뷰 데이터 전처리, 리뷰 최종 점수화·분석 테이블 생성, 사이트 구현** |
| 팀원 C | 리뷰 데이터 분석, 머신러닝 실험 |
| 팀원 D | 리뷰 데이터 분석, 머신러닝 실험 |

### 세부 담당 범위

- 리뷰 원문 데이터의 품질을 점검하고 후속 분석에 사용할 수 있도록 전처리했다.
- 리뷰 분석팀의 선행 집계 결과를 입력받아 평점 기반 기본 점수와 사용자 질문 Q1·Q2·Q3 점수를 결합했다.
- 리뷰 수에 따른 신뢰도 보정, 최종 점수·등급·요약 문구를 산출하고 `review_analysis_table.xlsx` 형태의 최종 리뷰 분석 테이블을 만들었다.
- 서비스 화면 구현에 참여하고 리뷰 분석 결과가 채용공고 분석 결과와 연결되도록 협업했다.

표준화·복합명사·카테고리·불용어 사전 정비, 영역별 신호 추출과 기업 단위 집계는 리뷰 분석 담당자들이 나누어 수행한 공동 작업이다. 따라서 이 저장소는 전체 팀 결과와 본인의 직접 담당 범위를 구분해 설명한다.
