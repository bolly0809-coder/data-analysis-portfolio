...
# -------------------------------------------------------------------
# Usage guides for notebook help
...
```python
my_plot.i              # 전체 plot 목록
my_plot.lineplot.i     # Line Plot
my_plot.kdeplot.i      # KDE Plot
my_plot.histplot.i     # Histogram
my_plot.boxplot.i      # Box Plot
my_plot.violinplot.i   # Violin Plot
my_plot.heatmap.i      # Heatmap
my_plot.barplot.i      # Bar Plot
my_plot.countplot.i    # Count Plot
my_plot.pieplot.i      # Pie / Donut Plot
my_plot.stackplot.i    # Stacked Bar Plot
my_plot.scatterplot.i  # Scatter / Convex Hull
my_plot.lmplot.i       # Lm Plot
my_plot.pairplot.i     # Pair Plot
```

공통 옵션:
- `data=df`: 사용할 DataFrame
- `x`, `y`: 시각화할 컬럼명
- `hue`: 범주 구분 컬럼명
- `title`, `xlabel`, `ylabel`: 제목과 축 이름
- `width`, `height`: 그래프 크기
- `palette`: 색상 팔레트
- `save_path`: 이미지 저장 경로
- `ax`: subplot이나 이중 축에 직접 그릴 때 사용할 축 객체
""")
...
def lineplot(data=None, x=None, y=None, hue=None,
    ...
    # 선 그래프 그리기
    sb.lineplot(data=data, x=x, y=y, hue=hue, color=color,
...
def kdeplot(data=None, x=None, hue=None, meanline=False, clevel=0, title=None, 
            xlabel=None, ylabel=None, fill=False, linewidth=2.0, palette=None, 
            width=1280, height=640, save_path=None, ax=None):
    """
...
def histplot(data=None, x=None, bins="auto", hue=None, 
    ...
    # 히스토그램 그리기
    sb.histplot(data=data, x=x, hue=hue, linewidth=linewidth, palette=palette, 
                kde=kde, bins=bins, ax=ax)
...
def heatmap(data=None, annot=True, fmt="0.2f", linewidth=0.5,
    ...
    # 히트맵 그리기
    sb.heatmap(data=data, annot=annot, fmt=fmt, linewidths=linewidth,
               cmap=palette, ax=ax, **params)
...
def barplot(data=None, x=None, y=None, hue=None, estimator=np.mean,
            ...
            width=1280, height=640, save_path=None, ax=None, errorbar='ci'):
    """
    막대그래프를 그린다.
    ...
    Args:
        ...
        height: 캔버스 세로 픽셀.
        save_path: 이미지 저장 경로.
        errorbar: 오차막대 표시 방식 (seaborn.barplot의 errorbar 인자, 기본값 'ci').
    """
    # 그래프 초기화
    fig = None
    if ax is None:
    ...
    sb.barplot(data=data, x=x, y=y, hue=hue, estimator=estimator,
               order=order, palette=palette, ax=ax, errorbar=errorbar)
...
def stackplot(data, x, y, hue, aggfunc=np.sum, orient='v', ratio=False,
    ...
    Args:
        ...
        orient: 막대 방향('v' 또는 'h') (기본값: 'v').
        ratio: 누적값을 비율로 표시할지 여부 (기본값: None).
        text: 누적값 텍스트 표시 여부 (기본값: True).
        text_color: 누적값 텍스트 색상 (기본값: "#ffffff").
        text_fontsize: 누적값 텍스트 폰트 크기 (기본값: 12).
        palette: 색상 팔레트 이름.
        title: 그래프 제목.
        xlabel: x축 레이블.
        ylabel: y축 레이블.
        width: 캔버스 가로 픽셀 (기본값: 1280).
        height: 캔버스 세로 픽셀 (기본값: 640). 
        save_path: 이미지 저장 경로.
    """
    # 그래프 초기화
    fig = None
...
def plot_hull(data, x, y, hue, palette, ax):
    ...
    for i, v in enumerate(classes):
        ...
        # convexHull은 3개 이상의 점이 필요하므로 데이터 포인트가 3개 미만이면 중단해야 함
        if len(df_c) < 3:
            continue
...
def _draw_ci(ax, interval, color, ymax):
    ...
    # 신뢰구간 범위에 대한 텍스트 추가
    ax.text(cmin, ymax * 0.9, f'{cmin:.2f}', color=color, ha='right')
    ax.text(cmax, ymax * 0.9, f'{cmax:.2f}', color=color, ha='left')
