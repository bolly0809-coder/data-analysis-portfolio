...
# 곡석이 꺽이는 지점(엘보우 포인트)을 계산해주는 패키지 
...
def best_k_elbow(data, klist=None, columns=None, scaling='standard',
    ...
    이너셔(군집 중심까지 거리의 제곱합)는 k가 커질수록 항상 줄어들기 때문에 가장 작은 값을
...
def best_k(data, klist=None, columns=None, scaling='standard',
    ...
    if verbose:
        ...
        print(f"[1단계] 엘보운 포인트       : k = {elbow_k}  (뭉침만 확인 -> 후보)")
        ...
        if elbow_k == sil_k:
            ...
            if not elbow_row.empty:
                print(f"    · k = {elbow_k} : 균형지수 {elbow_row.iloc[0]['균형지수']} "
                      f"(스코어 {elbow_row['스코어']}, 두께비 {sil_row['두께비']})")
...
def plot_dendrogram(estimator, cut_height=None, title=None,
    ...
    Args (기본값은 위의 함수 정의 참고):
        estimator: 학습이 끝난 AgglomerativeClustering 모델
        cut_height, title: 자른 높이(None이면 모델이 실제로 자른 높이를 되돌린다),
            그래프 제목(None이면 자동 생성)
        p, truncate_mode, leaf_rotation, leaf_font_size, count_sort:
            dendrogram()에 그대로 전달되는 잎(leaf) 표시 옵션들
        cut_line, cut_color: 자른 높이를 가로선으로 표시할지 여부, 그 선의 색상
        xlabel, ylabel: x·y축 이름(xlabel이 None이면 자동 생성)
        width, height, save_path, ax: 캔버스 가로·세로 픽셀, 저장 경로,
            그래프를 그릴 Axes 객체(None이면 새로 생성)
    """
...
def agglomerative(data, k=None, distance_threshold=None, columns=None, scaling='standard',
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        p, truncate_mode: 덴드로그램에 표시할 가지의 개수와 생략 방식
            ('lastp'=마지막 p개의 가지만, None이면 전체를 그린다)
    ...
    # comute_distances: 병합 거리를 남겨야 덴드로그램의 높이를 그릴 수 있다
    # comute_full_tree: 중간에 멈추면 나무의 윗부분이 잘려 덴드로그램이 불완전해진다
...
def dbscan(data, eps=0.5, min_samples=5, columns=None, scaling='standard',
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        outline: 군집의 외곽선(ConvexHull)을 표시할 여부
        palette, size, edgecolor, linewidth, alpha: 군집별 색상 팔레트(외곽 벡터·외곽선에도
            같이 적용), 마커 크기, 테두리 색상, 테두리 두께, 투명도
    ...
    # DBCAN 에는 predict()가 없다. 학습 결과는 labels_에 들어 있다
    labels = estimator.labels_

    # --- 4) 각 데이터의 군집 번호와 벡터 유형을 컬럼으로 추가 ---
    # 핵심(core) : 반영 안에 min_samples 개 이상을 거느린 데이터 (군집의 몸통)
    # 외곽(botder): 스스로는 기준에 못 미치지만 핵심의 반경 안에 있는 데이터 (군집의 가장자리)
    # 노이즈(noise): 어느 쪽도 아닌 데이터 (군집 번호가 -1)

    # 'border'라는  값으로 채운, 데이터 길이와 동일한 배열 생성 
    vectors = np.full(len(df), 'border', dtype=object)

    # core_sample_indices_ 는 "몇 번쨰 행"인지를 담은 위치 번호이므로 위치로 사용한다 
