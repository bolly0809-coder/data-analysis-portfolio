...
def reduce_vif(df, columns=None, threshold=10.0, verbose=True):
    """
    다중 공산성이 사라질 때까지 VIF가 가장 큰 변수를 하나씩 반복 제거하는 함수
    ...
    Returns:
        DataFrame: 대상 변수들의 VIF가 모두 threshold 미만이 되도록 일부 변수가 제거된 데이터프레임.
    ...
    while True:
        ...
        # 종료 조건: 가장 큰 VIF도 기준 미만이거나 남은 변수가 한 개라면 종료
...
def labeling(df, columns, save_path=None, verbose=True):
    ...
    # --- 1) 작업 준비 ---
    result = df.copy()      # 원본 보존을 위해 복사본으로 작업
...
def dummies(df, columns, drop_first=True, verbose=True):
    ...
    if verbose:
        for c in targets:
            # "원본에 없고 결과에만 있으면서 'c_'로 시작하는 컬럼"이 c로부터 생성된 더미이다.
            created = []
...
# =====================================================================
def log_transform(df, log_columns=None, log1p_columns=None, reflect_columns=None, 
                  verbose=True):
    """
    로그변환 함수
    ...
    Args:
        ...
        log_columns (list, optional): 순수 로그 변환할(우측꼬리, 0없음) 컬럼명 리스트 (기본값:None)
        log1p_columns (list, optional): log(1+x) 변환할(우측꼬리) 컬럼명 리스트 (기본값: None)
        reflect_columns (list, optional): 반사 후 로그 변환할(좌측꼬리) 컬럼명 리스트 (기본값:None)
    ...
    # --- 2) 우측꼬리 컬럼 변환 (1): log(x) ---
    # 0 이하의 값이 하나라도 있으면 -inf나 NaN이 되므로 미리 막는다
    if log_columns:
    ...
    if reflect_columns:
        for c in reflect_columns:
            ...
            report.append([c, '좌측 꼬리', f'log(1+{max_value:g}-x)', 
...
# 로그 역변환
# =====================================================================
def inverse_log_transform(df, log_columns=None, log1p_columns=None, reflect_columns=None, 
                  verbose=True):
    """
    ...
    Args:
        ...
        log_columns (list, optional): 순수 로그 변환했던(우측꼬리) 컬럼명 리스트 (기본값:None)
        log1p_columns (list, optional): log(1+x) 변환했던(우측꼬리) 컬럼명 리스트 (기본값: None)
        reflect_columns (list, optional): 반사 변환했던(좌측꼬리) 컬럼의
                        {컬럼명: 변환 당시의 최댓값} (예: {'B': 369.9}) (기본값:None)
...
def replace_outlier(df, columns=None, method='bound', value=None, verbose=True):
    ...
    # --- 2) 작업 준비 ---
    result = df.copy()      # 원본 보존을 위해 복사본으로 작ㅇ버
...
def scaling(df, columns=None, method='standard', save_path=None, verbose=True):
    ...
    Args:
        ...
        columns (list, optional): 스케일링할 컬럼명 리스트.
            None이면 df의 수치형 컬럼을 자동 선택한다 (기본값: None).
    ...
    # --- 3) 스케일러 학습 및 변환 ---
    # 이름에 해당하는 클래스로 스케일러를 만들어 대상 컬럼의 기준값(평균, 표준편차 등)을 학습시킨답
    scaler = SCALERS[name]()
...
def pca(df, y=None, n_components=0.8, scale=True, method='standard',
    ...
    # --- 6) 로딩 벡터표 출력 ---
    if report:
        # 6-1) 압축 결과 요약 (변수 몇 개 및 주성분 몇 개로 줄었는지)
