...
def test_assumptions(data, columns=None, alpha=0.05, center="median"):
    ...
    각 변수에 대해 정규성 검정(normaltest)를 수행하고, 변수가 두 개 이상인 경우
    ...
    Returns:
        DataFrame: field를 인덱스로 하는 검정 결과표 (test, statistic, p-value, result 컬럼 포함)
    """    
...
def test_1sample(data, column, popmean=0, alpha=0.05):
    ...
    정규성 충족 시 단일표본 t검정, 미충족 시 wilcoxon 부호순위 검정을 수행하며,
    양측, 좌측단측, 우측단측 3가지 대립가설을 일괄 검정한다.
...
def test_paired(data, before, after, alpha=0.05, plot=True, palette=None, title=None, 
                xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    """
    짝지어진 두 측정값(전/후)의 차이가 있는지 검정하는 함수 (wide 형식)

    차이값 d = after - before의 정규성 충족 시 대응표본 t검정,
    ...
    Returns:
        DataFrame: 대립가설(alternative)별 검정/통계량/p-value/유의성 결과표
    """
    # 같은 행끼리 짝지어야 하므로 두 컬럼을 함께 결측행 제거
    ...
    for alt in ("two-sided", "less", "greater"):
        ...
        else:           # 정규성 미충족 - > Wilcoxon 부호순위 검정
            stat, p = wilcoxon(paired[after], paired[before], alternative=alt)
        
        significant = p < alpha     # p < alpha이면 통계적으로 유의 (귀무가설 기각)
...
def test_independent(data, group1, group2, alpha=0.05, plot=True, palette=None, title=None, 
                     xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    """
    독립된 두 집단의 평균이 같은지 검정하는 함수

    두 집단 모두 정규성 충족 시 등분산성에 따라 student/welch t 검정을,
    ...
    # 각 집단의 컬럼을 분리하고 결측 제거 (독립표본이므로 컬럼별로 따로 제거)
    ...
    if plot:
        ...
        annotator = Annotator(data=melt_df,             # 데이터프레임
                              x='group', y='value',     # x축, y축 변수 지정
                              pairs=[(lv[0], lv[1])],   # 비교할 그룹 쌍
                              ax=ax)                    # 그래프 축
        
        # 가설검정 알고리즘 종류 (적용된 검정에 맞춰 지정)
        if test_name == "Mann-Whitney U test":   
...
def anova_oneway(data, y, between, alpha=0.05):
    ...
    # 어떤 검정을 사용했는지 식별할 수 있도록 맨앞에 test 컬럼 추가
...
def posthoc_oneway(data, y, between, alpha=0.05, plot=True, summary=True, palette=None, title=None,
                   xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    """
    일원분산분석(One-way ANOVAd)의 사후검정을 수행하는 함수
    ...
    if plot:
        fig, ax = my_plot.init(title=title, width=width, height=height, xlabel=xlabel,
                               ylabel=ylabel)
        my_plot.boxplot(data=df, x=between, y=y, hue=between, palette=palette, order=order,
                        ax=ax)        
...
def anova_twoway(data, y, between, alpha=0.05, order=None, plot=True, palette=None, 
    ...
    if plot:
        ...
        # (title은 전체 제목(subtitle)이며, 지정하지 않으면 서브플롯별 제목만 표시된다.)
        ...
        for i in range(2):
            ...
            my_plot.pointplot(data=df, x=x_field, y=y, hue=hue_field, order=x_order,
                            hue_order=hue_order, errorbar="se", capsize=0.1, palette=palette,
...
def posthoc_twoway(data, y, between, alpha=0.05, plot=True, summary=True, palette=None, 
    ...
    # 어떤 사후검정을 사용했는지 식별할 수 있도록 맨앞에 test 컬럼 추가
    result.insert(0, "test", posthoc_name)
    # p값이 유의수준 미만이면 통계적으로 유의 (귀무가설 기각)
    ...
    if plot:
        ...
        my_plot.boxplot(data=df, x=group, y=y, order=order, palette=palette, title=title,
                        xlabel=xlabel, ylabel=ylabel, width=width, height=height,
                        save_path=save_path)  
...
def correlation(data, x, y, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, 
    ...
    Returns:
        DataFrame: (x,y)를 인덱스로 하는 단일행 결과표
    """

    # --- 1) 같은 행끼리 비교해야 하므로 두 컬럼을 함께 결측행 제거---
    ...
    # --- 6) 상관 강도 해석 라벨 ---
    # |r| > 0.7 → 강함, 0.3 < |r| <= 0.7 → 중간, 0 < |r| <= 0.3 → 약함, 그 외 → 없음
    a = abs(coef)
    ...
    # --- 7) 가정 점검 결과와 선택된 상관계수를 단일행 결과표로 정리 ---
    row = {
        "x": x,
        "y": y,
        "method": method,
    ...
    if plot:
        my_plot.scatterplot(data=pair, x=x, y=y, palette=palette, title=title,
                            xlabel=xlabel, ylabel=ylabel, width=width, height=height,
                            save_path=save_path)
...
def multi_correlation(data, columns=None, alpha=0.05, plot=True, palette=None, title=None, 
                      diag_kind="kde", reg=False, width=1280, height=1024, save_path=None):
    """
    여러 변수 쌍에 대한 상관분석을 일괄 수행하고 결과를 출력하는 함수

    Args:
        data (DataFrame): 분석 대상 데이터프레임
        columns (list): 분석에 사용할 컬럼명 목록 (기본값: None → 수치형 컬럼 전체)
        alpha (float): 유의수준 (기본값: 0.05)
    ...
    # --- 2) 변수 조합 쌍별 상관분석 ---
    # 각 컬럼 조합에 대해 correlation()을 호출하여 단일행 결과를 세로로 누적
    ...
    # --- 4) 산점도 행렬 시각화---
    if plot:
        my_plot.pairplot(data=data[columns], palette=palette, diag_kind=diag_kind, reg=reg,
                         title=title, width=width, height=height, save_path=save_path)
...
def compute_vif(df, columns=None):
    ...
    if columns is None:
        ...
        # 원본 데이터프레임에서 columns에 해당하는 컬럼만 추출
        target = df[columns]

        # VIF는 수치 행렬 연산이므로 비수치형이 섞이면 안 된다
...
def chi2_goodness_of_fit(data, column, expected=None, order=None, alpha=0.05, plot=True,
...
def _chi2_crosstab(data, row, col, kind, alpha=0.05, plot=True, 
                   palette=None, orient='v', title=None, xlabel=None, ylabel=None, 
                   width=1280, height=640, save_path=None):
    """
    독립성 검정의 공통 처리부
    (가정확인 → 카이제곱/피셔 분기 → 크라메르 V)
    ...
    Returns:
        DataFrame: (row, col)을 인덱스로 하는 단일 행 결과표
    """
    ...
    result_df = DataFrame([{
        ...
        "chi2": (
            np.nan
            if np.isnan(chi2_out)
            else round(float(chi2_out), 4)
        ),
        "dof": dof_out,
        "p-value": round(float(p), 6),
        "significant": bool(p < alpha),
...
def chi2_homogeneity(data, group, category, alpha=0.05, plot=True, palette=None, 
                     orient='v', title=None, xlabel=None, ylabel=None,
                      width=1280, height=640, save_path=None):
    """
    ...
    return _chi2_crosstab(data, group, category, "homogeneity", alpha, plot=plot, 
                          palette=palette, orient=orient, title=title, xlabel=xlabel, 
...
def correlation_summary(corr_df, strength="Strong"):
    ...
    # --- 2) 변수별 최강 상대 ---
    # 절대값이 큰 순으로 정렬했으므로 그룹별 첫번째 항목이 곧 최강 상대가 된다.
    # 강한 쌍이 없는 변수도 "중복없음"이라는 근거로 남아야 하므로 여기서는 강도로 거르지 않는다.
    ...
    # -> 함수 호출 시 ['Strong', 'Moderate'] 같이 여러 강도를 지정할 수 있도록 하기 위함
